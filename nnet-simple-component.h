...
/* LayerScaleComponent */
 class LayerScaleComponent: public UpdatableComponent {
  public:
   virtual int32 InputDim() const { return scales_.Dim(); }
   virtual int32 OutputDim() const { return scales_.Dim(); }
   ...
   LayerScaleComponent() { } // use Init to really initialize.
   virtual std::string Type() const { return "LayerScaleComponent"; }
   virtual int32 Properties() const {
     return kSimpleComponent|kUpdatableComponent|kBackpropNeedsInput|
         kPropagateInPlace|kBackpropInPlace;
   }
  ...
  // Some functions that are specific to this class.
  explicit LayerScaleComponent(const LayerScaleComponent &other);
 ...
 protected:
  ...
  virtual void Update(
      ...
      const CuMatrixBase<BaseFloat> &in_value,
      const CuMatrixBase<BaseFloat> &out_deriv) {
    UpdateSimpleMod(in_value, out_deriv);
  }
  

  const LayerScaleComponent &operator
      = (const LayerScaleComponent &other); // Disallow.
  CuVector<BaseFloat> scales_;
