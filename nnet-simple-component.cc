...
void LayerScaleComponent::Scale(BaseFloat scale) {
  if (scale == 0.0) {
    scales_.SetZero();
  } else {
    scales_.Scale(scale);
...
void LayerScaleComponent::Add(BaseFloat alpha,
                                   const Component &other_in) {
  const LayerScaleComponent *other =
      dynamic_cast<const LayerScaleComponent*>(&other_in);
  KALDI_ASSERT(other != NULL);
  scales_.AddVec(alpha, other->scales_);
}

LayerScaleComponent::LayerScaleComponent(
    const LayerScaleComponent &component):
    UpdatableComponent(component),
    scales_(component.scales_) { }
void LayerScaleComponent::PerturbParams(BaseFloat stddev) {
  CuVector<BaseFloat> temp_scales(scales_.Dim(), kUndefined);
  temp_scales.SetRandn();
  scales_.AddVec(stddev, temp_scales);
}
std::string LayerScaleComponent::Info() const {
  std::ostringstream stream;
  stream << UpdatableComponent::Info()
         << ", scales-min=" << scales_.Min()
         << ", scales-max=" << scales_.Max();
...
Component* LayerScaleComponent::Copy() const {
  return new LayerScaleComponent(*this);
}
BaseFloat LayerScaleComponent::DotProduct(
    const UpdatableComponent &other_in) const {
  const LayerScaleComponent *other =
      dynamic_cast<const LayerScaleComponent*>(&other_in);
  return VecVec(scales_, other->scales_);
}
void LayerScaleComponent::Init(int32 dim,
                                    BaseFloat param_mean,
                                    BaseFloat param_stddev) {
  KALDI_ASSERT(dim > 0 && param_stddev >= 0.0);
  this->diml = dim;
  KALDI_LOG << "diml set to " << diml;
  scales_.Resize(dim);
  scales_.SetRandn();
  scales_.Scale(param_stddev);
  scales_.Add(param_mean);
}
void LayerScaleComponent::Init(std::string vector_filename) {
  CuVector<BaseFloat> vec;
  ReadKaldiObject(vector_filename, &vec); // will abort on failure.
  scales_.Resize(vec.Dim());
  scales_.CopyFromVec(vec);
}
void LayerScaleComponent::InitFromConfig(ConfigLine *cfl) {
  std::string vector_filename;
  int32 dim = -1;
  InitLearningRatesFromConfig(cfl);
  if (cfl->GetValue("vector", &vector_filename)) {
...
void* LayerScaleComponent::Propagate(
    const ComponentPrecomputedIndexes *indexes,
    const CuMatrixBase<BaseFloat> &in,
    CuMatrixBase<BaseFloat> *out) const {
  out->CopyFromMat(in);
...
void LayerScaleComponent::UpdateSimpleMod(
    const CuMatrixBase<BaseFloat> &in_value,
    const CuMatrixBase<BaseFloat> &out_deriv) {
  ...
  */
  ...
  /* Author:: Minali */
  //calculate the average of the multiplication
  //KALDI_LOG << "From UpdateSimpleMod:: diml = " << diml;
  CuVector<BaseFloat> change_scales_(scales_.Dim(), kSetZero);
  change_scales_.AddDiagMatMat(learning_rate_, out_deriv, kTrans,
                        in_value, kNoTrans, 1.0);
  scales_.Add(change_scales_.Sum()/scales_.Dim());

  /* Author:: Minali */

}

void LayerScaleComponent::Backprop(
    const std::string &debug_info,
    const ComponentPrecomputedIndexes *indexes,
    const CuMatrixBase<BaseFloat> &in_value,
    const CuMatrixBase<BaseFloat> &, // out_value
    ...
    Component *to_update_in,
    CuMatrixBase<BaseFloat> *in_deriv) const {
  LayerScaleComponent *to_update =
      dynamic_cast<LayerScaleComponent*>(to_update_in);

  if (to_update != NULL) {
    // Next update the model (must do this 2nd so the derivatives we propagate
    // are accurate, in case this == to_update_in.)
    if (to_update->is_gradient_)
      to_update->UpdateSimpleMod(in_value, out_deriv);
    else  // the call below is to a virtual function that may be re-implemented
      to_update->Update(debug_info, in_value, out_deriv);  // by child classes.
  }
...
void LayerScaleComponent::Read(std::istream &is, bool binary) {
  ReadUpdatableCommon(is, binary);  // Read opening tag and learning rate.
  ExpectToken(is, binary, "<Params>");
  scales_.Read(is, binary);
  if (PeekToken(is, binary) == 'I') {
  ...
  ExpectToken(is, binary, "</LayerScaleComponent>");
}

void LayerScaleComponent::Write(std::ostream &os, bool binary) const {
  WriteUpdatableCommon(os, binary);  // Write opening tag and learning rate.
  WriteToken(os, binary, "<Params>");
  scales_.Write(os, binary);
  WriteToken(os, binary, "</LayerScaleComponent>");
}
int32 LayerScaleComponent::NumParameters() const {
  return InputDim();
}
void LayerScaleComponent::Vectorize(VectorBase<BaseFloat> *params) const {
  params->CopyFromVec(scales_);
}
void LayerScaleComponent::UnVectorize(
    const VectorBase<BaseFloat> &params) {
  scales_.CopyFromVec(params);
}
